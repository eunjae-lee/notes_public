...
gh api "search/issues?q=author:$USERNAME+created:$MONDAY..$TODAY+type:pr+org:calcom" --jq '.items[] | "• " + (.created_at | split("T")[0]) + " - " + .title + " (" + (.repository_url | split("/") | .[-1]) + ") - " + .html_url'
echo ""

echo "💬 Comments Made This Week:"
gh api "search/issues?q=commenter:$USERNAME+created:$MONDAY..$TODAY+org:calcom" --jq '.items[] | "• " + (.created_at | split("T")[0]) + " - " + .title + " (" + (.repository_url | split("/") | .[-1]) + ") - " + .html_url'
echo ""
...
gh api "search/issues?q=author:$USERNAME+updated:$MONDAY..$TODAY+involves:$USERNAME+org:calcom" --jq '.items[] | "• " + (.updated_at | split("T")[0]) + " - " + .title + " (" + (.repository_url | split("/") | .[-1]) + ") - " + .html_url'
echo ""
...
while true; do
    ...
    pr_count=$(gh api "search/issues?q=author:$USERNAME+created:$current_date+type:pr+org:calcom" --jq '.total_count')
    if [[ $pr_count -gt 0 ]]; then
        echo "  📝 PRs created: $pr_count"
    fi
    
    # Comments made on this day
    comment_count=$(gh api "search/issues?q=commenter:$USERNAME+created:$current_date+org:calcom" --jq '.total_count')
